<div class="row">
  ...
  {{-- <div class="form-group col-6">
    <label for="current_round">{{__('bolao.current_round')}}</label>
    <input type="text" class="form-control{{ $errors->has('current_round') ? ' is-invalid' : '' }}" name="current_round" value="{{ old('current_round') ?? ($register->current_round ?? '') }}">
    @if ($errors->has('current_round'))
        <span class="invalid-feedback" role="alert">
            <strong>{{ $errors->first('current_round') }}</strong>
        </span>
    @endif
  </div> --}}
